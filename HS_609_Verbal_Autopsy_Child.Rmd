...
Plot grouped box plots for numerical variables with the top 5 cause of death (across the whole dataset)
```{r fig.width = 10}
var_list <- names(training_set_copy_top5)
...
Plot grouped bar charts for categorical variables with the top 5 cause of death (across the whole dataset) (just an example, and will put it in loop later)
```{r fig.width = 10}
grouped_class <- training_set_copy_top5 %>% 
  group_by(site,Gender,gs_text34) %>% 
  dplyr::summarise(count=n()) %>% 
  mutate(perc=count/sum(count))

grouped_bar <- ggplot(data=grouped_class, aes(x=Gender, y=perc, fill=gs_text34)) + 
  geom_bar(position = 'dodge', stat ='identity')+scale_y_continuous(labels = scales::percent)+labs(x = "medical_specialty", y = "Percentage of Readmission", fill = "Readmission") + ggtitle(labs(title = "Distribution of medical_specialty by Readmission"))+ theme(text = element_text(size=12))+ geom_text(aes(label = scales::percent(perc),y = perc),position=position_dodge(width=1), vjust = -0.25,size=3.5)+ facet_grid(. ~ site,scales='free')
...
gen <- ggplot(data = training_set_copy) + 
  geom_bar(aes(x = c1_09, fill = c1_09), stat = "Count")
print(gen)
```
